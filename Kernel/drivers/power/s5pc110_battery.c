...
/* 20100823 hongkuk.son for 507211 */
...
static int s3c_get_bat_temp(void)
{
	...
	//disable temp. exception case
	//ex_case = OFFSET_MP3_PLAY | OFFSET_VOICE_CALL_2G | OFFSET_VOICE_CALL_3G	| OFFSET_VIDEO_PLAY;
...
/* 20100823 hongkuk.son for 507211 */
	if( ( HWREV >= 0x2 ) && ( HWREV != 0xE ) )
	{
		if( ( s3c_bat_info.bat_info.level >= 16 ) && ( rcomp_status != 0 ) )
			fuel_gauge_rcomp ( FUEL_INT_1ST );
		else if( ( s3c_bat_info.bat_info.level < 16 ) && ( s3c_bat_info.bat_info.level >= 6 ) && ( rcomp_status != 1 ) )
			fuel_gauge_rcomp ( FUEL_INT_2ND );
		else if( ( s3c_bat_info.bat_info.level < 6 ) && ( rcomp_status != 2 ) )
...
static void s3c_low_bat_work ( struct work_struct * work )
{
	...
	{
		printk ( "@@@@@     %s : Can't read soc!!\n", __func__ );
		temp_soc = s3c_bat_info.bat_info.level;
	}

	printk("@@@@@     %s : rcomp_status= %d, temp_soc= %d\n", __func__, rcomp_status, temp_soc );
...
/* 20100823 hongkuk.son for 507211 */
	...
	if( ( HWREV >= 0x2 ) && ( HWREV != 0xE ) )
	{
		s3c_gpio_cfgpin ( GPIO_KBR3, S5PV210_GPH3_3_EXT_INT33_3 );	// 20100824 hongkuk.son Fix Build Error
		...
		if( ret == 0 )
			printk ( "@@@@@     %s : low battery interrupt registered\n", __func__ );
		else
		{
