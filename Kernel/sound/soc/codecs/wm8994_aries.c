...
// Headset
#define TUNING_RING_OUTPUTL_VOL		0x38 //  0x34		// 1Ch
#define TUNING_RING_OUTPUTR_VOL		0x38 //  0x34		// 1Dh
...
void audio_ctrl_earmic_bias_gpio(int enable)
{
	...
	if(enable)
	{
		DEBUG_LOG("enable = [%d]", enable);
		set_recording_status(1);
		gpio_set_value(GPIO_EARMICBIAS_EN, 1);
	}
	else
	{
		set_recording_status(0);
...
void wm8994_disable_rec_path(struct snd_soc_codec *codec, enum mic_path rec_path) 
{
	...
	if(!(wm8994->codec_state & CALL_ACTIVE))	// Normal case
	{
		...
		audio_ctrl_earmic_bias_gpio(0);	// Disable EARMIC bias
#endif
	}
...
void wm8994_set_voicecall_receiver(struct snd_soc_codec *codec)
{
	...
	// HPF
	wm8994_write(codec,WM8994_AIF2_ADC_FILTERS, 0x7800 ); 
